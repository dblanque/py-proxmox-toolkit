...
def main(argv_a, **kwargs):
	...
	if argv_a.dry_run:
		logger.info("Running in dry-run mode. Commands will only be printed.")
	...
	for guest_type in all_guests:
		for guest_id in all_guests[guest_type]:
			...
			if guest_host != hostname:
				...
				guest_net_orig[int(guest_id)] = parse_guest_net_cfg(
					guest_id, debug=argv_a.debug
				)

	if argv_a.print_original:
		logger.info(
			"If you need to restore the config manually, here's the output to use in the cfg python file."
		)
		logger.info(guest_net_orig)

	if len(guest_net_map) < 1 or (
		len(all_guests["vm"]) < 1 and len(all_guests["ct"]) < 1
	):
		...
		for net_id, net_opts in net_cfg.items():
			...
			if guest_is_ct:
				cmd_args = [
					...
					str(guest_id),
					f"--net{net_id}",
					parse_net_opts_to_string(net_opts),
				]
			else:
				cmd_args = [
					"/usr/sbin/qm",
	...
	if not argv_a.rollback_delay:
		...
		rollback_delay = argv_a.rollback_delay
