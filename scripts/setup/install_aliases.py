...
from core.templates.shell.py_proxmox_aliases import (
	TEMPLATE_ALIASES,
	TEMPLATE_COMPLETION,
)
...
def install_actsh(toolkit_path, act_sh_path):
	if os.path.islink(act_sh_path) or os.path.isfile(act_sh_path):
		if not os.path.exists(act_sh_path + ".old"):
			...
			print_c(
				bcolors.L_YELLOW,
				f"{act_sh_path} was renamed to {act_sh_path}.old",
			)
	elif os.path.exists(act_sh_path):
		raise ValueError(
			act_sh_path + "already exists and is not a link or a file?"
		)
	with open(act_sh_path, "w") as act_sh:
		act_sh.write(SCRIPT_TEMPLATE.format(toolkit_path=toolkit_path).lstrip())
...
def install_aliases(user_home, bash_completion, add_aliases):
	if bash_completion is not True and add_aliases is not True:
		return
	...
	with open(ALIASES_PATH, "w") as aliases_file:
		...
		if bash_completion:
			...
			print_c(
				bcolors.L_YELLOW, f"Wrote bash-completion to {ALIASES_PATH}"
			)
...
def main(argv_a: LocalParser, **kwargs):
	...
	install_aliases(
		user_home=user_home,
		bash_completion=argv_a.bash_completion,
		add_aliases=argv_a.add_aliases,
	)
