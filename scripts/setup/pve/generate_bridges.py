...
def main(argv_a, **kwargs):
	...
	physical_interfaces = get_interfaces(
		interface_patterns=PHYSICAL_INTERFACE_PATTERNS
	)
	...
	if not argv_a.keep_offloading:
		...
		try:
			...
			raise Exception(
				"Please install ethtool or use option -x (see help)."
			)
	...
	with open(NEW_INTERFACES_FILE, "w") as f:
		...
		if argv_a.ovs_bridge:
			...
			for iface in physical_interfaces:
				...
				if (
					current_bridge not in configured_ifaces
					and not parent_bridge
				):
					print_c(
						bcolors.L_YELLOW,
					...
					configured_ifaces[current_bridge]["bridge-ports"].append(
						iface
					)
				else:
					print_c(
				...
				if (
					argv_a.keep_offloading is not True
					and iface in configured_ifaces
				):
					if "post-up" not in configured_ifaces[iface]:
						...
						configured_ifaces[iface]["post-up"] = (
							OFFLOADING_CMD.format(iface).split()
						)
					else:
						print_c(
							bcolors.L_RED,
				...
				if iface in configured_ifaces and not argv_a.reconfigure_all:
					...
					print_c(
						bcolors.L_GREEN,
						f"{INDENT}Interface {iface} configured.",
					)
