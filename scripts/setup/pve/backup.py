...
def argparser(**kwargs) -> ArgumentParser:
	...
	parser.add_argument(
		...
		"--output-path",
		default="/opt",
		help="Backup Tar output path, use without trailing slash.",
	)
	parser.add_argument(
		"-e", "--extra-dirs", help="Extra directories to backup.", nargs="+"
	)
	return parser
...
def main(argv_a, **kwargs):
	...
	try:
		...
		cmd: list = (
			f"tar -czvf {argv_a.output_path}/pve-bkp-{backup_date_fmted}.tar.gz --absolute-names".split()
			+ tar_dirs
		)
		...
		print_c(
			bcolors.L_GREEN,
			f"Backup Completed in {round(timer_e - timer_s, 3)} seconds",
		)
	except:
