...
def argparser(**kwargs) -> ArgumentParser:
	...
	parser.add_argument(
		"-f",
		"--fields",
		help="Use specific UDEV Info Properties as identifier if available instead of MAC Address.",
		nargs="+",
		default=None,
	)
	parser.add_argument(
		"-o",
		"--overwrite",
		help="Over-write pre-existing UDEV Link Files.",
		action="store_true",
	)
	parser.add_argument(
		"-p",
		"--print",
		help="Print data instead of writing to UDEV Link Files.",
		action="store_true",
	)
...
def main(argv_a, **kwargs):
	...
	interfaces = argv_a.interface or get_interfaces(
		interface_patterns=regex_list, verbose=argv_a.verbose
	)
	for iface_name in interfaces:
		iface_mac_addr = None
		udev_link_name = os.path.join(UDEV_PATH, f"10-{iface_name}.link")
		with subprocess.Popen(
			f"cat /sys/class/net/{iface_name}/address".split(),
			...
			print(
				f"Interface {colorize(bcolors.L_BLUE, iface_name)} will be pinned with MAC Address {colorize(bcolors.L_RED, iface_mac_addr)}"
			)
			if os.path.isfile(udev_link_name) and not use_overwrite:
				print(
					f"UDEV Link File {udev_link_name} for Interface {iface_name} already exists, skipping.\n"
				)
				continue
			...
			if udev_fields and any([f in udev_info for f in udev_fields]):
				...
				data = UDEV_BY_MAC_ADDRESS.format(
					iface_name=iface_name, iface_mac_addr=iface_mac_addr
				).strip()
			if use_print:
				print_c(
					bcolors.L_YELLOW,
	...
	print(
		"You may execute the command {0} to refresh the Interface UDEV Links.".format(
