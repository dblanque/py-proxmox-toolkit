...
DEFAULT_CHOICES: YesNoChoicesDict = {"yes": ["yes", "y"], "no": ["no", "n"]}
...
def yes_no_input(
	...
	if show_choices:
		choices_str = (
			",".join(input_choices["yes"]) + "|" + ",".join(input_choices["no"])
		)
	...
	if input_default is not None:
		if (
			input_default.lower() in input_choices["yes"]
...
def prompt_reboot():
	if yes_no_input(msg="Do you wish to reboot now?", input_default=False):
