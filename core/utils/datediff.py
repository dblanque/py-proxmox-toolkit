...
parser.add_argument('-us', '--microsecond-precision', required=False, default=False, action="store_true")
parser.add_argument('-t', '--date-target',
...
def check_date_threshold():
	...
	try: date_start = datetime.strptime(args.date_start, DATE_FMT).astimezone(timezone.utc)
	except: raise ValueError(f"date_start | Incorrect Date Format, must be {DATE_FMT}")
	if not isinstance(args.date_target, datetime):
		try: date_target = datetime.strptime(args.date_target, DATE_FMT).astimezone(timezone.utc)
		except: raise ValueError(f"date_target | Incorrect Date Format, must be {DATE_FMT}")
	else:
